EESchema Schematic File Version 4
LIBS:bluetoothkiste-cache
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector:Conn_01x04_Female J6
U 1 1 5C93D289
P 7450 4950
F 0 "J6" H 7400 5300 50  0000 L CNN
F 1 "Conn_01x04_Female" H 7100 5200 50  0000 L CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x04_P2.54mm_Horizontal" H 7450 4950 50  0001 C CNN
F 3 "~" H 7450 4950 50  0001 C CNN
	1    7450 4950
	-1   0    0    1   
$EndComp
$Comp
L Connector:Screw_Terminal_01x03 J2
U 1 1 5C93D920
P 3750 6000
F 0 "J2" H 3850 6050 50  0000 L CNN
F 1 "Screw_Terminal_01x03" H 3850 5950 50  0000 L CNN
F 2 "TerminalBlock_MetzConnect:TerminalBlock_MetzConnect_Type059_RT06303HBWC_1x03_P3.50mm_Horizontal" H 3750 6000 50  0001 C CNN
F 3 "~" H 3750 6000 50  0001 C CNN
	1    3750 6000
	1    0    0    -1  
$EndComp
$Comp
L Connector:RJ45 J4
U 1 1 5C93DA28
P 4100 4850
F 0 "J4" H 3770 4854 50  0000 R CNN
F 1 "RJ45" H 3770 4945 50  0000 R CNN
F 2 "JS_Local:RJ45_SEGOR" V 4100 4875 50  0001 C CNN
F 3 "~" V 4100 4875 50  0001 C CNN
	1    4100 4850
	-1   0    0    1   
$EndComp
$Comp
L Regulator_Linear:L7805 U1
U 1 1 5C9417FB
P 5400 4300
F 0 "U1" H 5400 4542 50  0000 C CNN
F 1 "L7805" H 5400 4451 50  0000 C CNN
F 2 "Package_TO_SOT_THT:TO-220-3_Vertical" H 5425 4150 50  0001 L CIN
F 3 "http://www.st.com/content/ccc/resource/technical/document/datasheet/41/4f/b3/b0/12/d4/47/88/CD00000444.pdf/files/CD00000444.pdf/jcr:content/translations/en.CD00000444.pdf" H 5400 4250 50  0001 C CNN
	1    5400 4300
	1    0    0    -1  
$EndComp
$Comp
L Device:C C1
U 1 1 5C941928
P 4850 4550
F 0 "C1" H 4965 4596 50  0000 L CNN
F 1 "10u" H 4965 4505 50  0000 L CNN
F 2 "Capacitor_THT:C_Rect_L7.0mm_W2.0mm_P5.00mm" H 4888 4400 50  0001 C CNN
F 3 "~" H 4850 4550 50  0001 C CNN
	1    4850 4550
	1    0    0    -1  
$EndComp
$Comp
L Device:C C2
U 1 1 5C9419B5
P 5800 4550
F 0 "C2" H 5915 4596 50  0000 L CNN
F 1 "10u" H 5915 4505 50  0000 L CNN
F 2 "Capacitor_THT:C_Rect_L7.0mm_W2.0mm_P5.00mm" H 5838 4400 50  0001 C CNN
F 3 "~" H 5800 4550 50  0001 C CNN
	1    5800 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	5700 4300 5800 4300
Wire Wire Line
	4850 4300 4850 4400
Wire Wire Line
	4850 4300 5100 4300
$Comp
L power:+5V #PWR0101
U 1 1 5C941C8E
P 5800 4300
F 0 "#PWR0101" H 5800 4150 50  0001 C CNN
F 1 "+5V" H 5815 4473 50  0000 C CNN
F 2 "" H 5800 4300 50  0001 C CNN
F 3 "" H 5800 4300 50  0001 C CNN
	1    5800 4300
	1    0    0    -1  
$EndComp
Connection ~ 4850 4300
Text Notes 7250 4775 0    50   ~ 0
VCC
Text Notes 7250 4875 0    50   ~ 0
GND
Text Notes 7250 4975 0    50   ~ 0
TX
Text Notes 7250 5075 0    50   ~ 0
RX
Text Notes 7150 4650 0    50   ~ 0
HC-06
Wire Wire Line
	1700 5300 1400 5300
Wire Wire Line
	3700 5250 3400 5250
Wire Wire Line
	3700 5150 3400 5150
Wire Wire Line
	3400 5150 3400 5250
Wire Wire Line
	3700 4850 3400 4850
Wire Wire Line
	3400 4850 3400 5150
Connection ~ 3400 5150
NoConn ~ 3700 4750
Wire Wire Line
	3700 4650 3600 4650
Wire Wire Line
	3700 4550 3600 4550
Wire Wire Line
	3600 4550 3600 4650
Connection ~ 3600 4650
Wire Wire Line
	3600 4650 3400 4650
Wire Wire Line
	3700 4950 2750 4950
Text Label 2750 4950 0    50   ~ 0
FLARM-TX
Wire Wire Line
	3700 5050 2750 5050
Text Label 2750 5050 0    50   ~ 0
FLARM-RX
Wire Wire Line
	750  4900 1700 4900
Text Label 750  4900 0    50   ~ 0
LX-TX
NoConn ~ 1700 4500
NoConn ~ 1700 4600
NoConn ~ 1700 4800
NoConn ~ 1700 5000
NoConn ~ 1700 5100
NoConn ~ 1700 5200
Wire Wire Line
	3550 6550 2950 6550
Wire Wire Line
	3550 6650 2950 6650
Wire Wire Line
	3550 6750 2950 6750
Wire Wire Line
	3550 6850 2950 6850
Wire Wire Line
	3550 6950 2950 6950
Wire Wire Line
	3550 7050 2950 7050
Text Label 2950 6650 0    50   ~ 0
COMMON-RX
Text Label 2950 6550 0    50   ~ 0
COMMON-TX
Text Label 2950 6750 0    50   ~ 0
FLARM-TX
Text Label 2950 6850 0    50   ~ 0
FLARM-RX
Text Label 2950 6950 0    50   ~ 0
LX-TX
Text Label 2950 7050 0    50   ~ 0
LX-RX
Wire Wire Line
	3550 5900 3250 5900
Wire Wire Line
	3550 6000 3250 6000
Wire Wire Line
	3550 6100 3250 6100
$Comp
L power:GND #PWR0102
U 1 1 5C95B9F2
P 3250 5900
F 0 "#PWR0102" H 3250 5650 50  0001 C CNN
F 1 "GND" V 3255 5772 50  0000 R CNN
F 2 "" H 3250 5900 50  0001 C CNN
F 3 "" H 3250 5900 50  0001 C CNN
	1    3250 5900
	0    1    1    0   
$EndComp
$Comp
L power:+12V #PWR0103
U 1 1 5C95BA3F
P 3250 6000
F 0 "#PWR0103" H 3250 5850 50  0001 C CNN
F 1 "+12V" V 3265 6128 50  0000 L CNN
F 2 "" H 3250 6000 50  0001 C CNN
F 3 "" H 3250 6000 50  0001 C CNN
	1    3250 6000
	0    -1   -1   0   
$EndComp
$Comp
L power:+12P #PWR0104
U 1 1 5C95BA9E
P 3250 6100
F 0 "#PWR0104" H 3250 5950 50  0001 C CNN
F 1 "+12P" V 3265 6227 50  0000 L CNN
F 2 "" H 3250 6100 50  0001 C CNN
F 3 "" H 3250 6100 50  0001 C CNN
	1    3250 6100
	0    -1   -1   0   
$EndComp
Text Notes 2950 6350 0    50   ~ 0
+12P = Geschaltete 12V, um das Bluetooth-Modul \nausschalten zu können
Text Notes 2950 7500 0    50   ~ 0
COMMON-TX sollte über einen Umschalter mit\nFLARM-TX bzw. LX-TX verbunden werden, \num zwischen Flarm und LX wählen zu können. \nGenauso COMMON-RX mit FLARM-/LX-RX.
$Comp
L Interface_UART:MAX232 U2
U 1 1 5C9C01BB
P 9550 4900
F 0 "U2" H 9900 6100 50  0000 C CNN
F 1 "MAX232" H 10000 6000 50  0000 C CNN
F 2 "Package_DIP:DIP-16_W7.62mm" H 9600 3850 50  0001 L CNN
F 3 "http://www.ti.com/lit/ds/symlink/max232.pdf" H 9550 5000 50  0001 C CNN
	1    9550 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	5400 4600 5400 5500
Connection ~ 5400 5500
Wire Wire Line
	4850 4700 4850 5500
Connection ~ 4850 5500
Wire Wire Line
	4850 5500 5400 5500
Wire Wire Line
	3400 5250 3400 5500
Connection ~ 3400 5250
Connection ~ 3400 5500
Wire Wire Line
	3400 5500 4850 5500
Wire Wire Line
	8100 5500 8100 6200
Wire Wire Line
	8100 6200 9550 6200
Wire Wire Line
	9550 6200 9550 6100
Wire Wire Line
	8100 3600 8100 4300
$Comp
L power:GND #PWR0105
U 1 1 5CA1B1E1
P 750 5500
F 0 "#PWR0105" H 750 5250 50  0001 C CNN
F 1 "GND" H 755 5327 50  0000 C CNN
F 2 "" H 750 5500 50  0001 C CNN
F 3 "" H 750 5500 50  0001 C CNN
	1    750  5500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4550 4300 3400 4300
Wire Wire Line
	3400 4650 3400 4300
Connection ~ 3400 4300
Wire Wire Line
	3400 4300 750  4300
$Comp
L power:+12V #PWR0106
U 1 1 5CA1F7A5
P 4550 4300
F 0 "#PWR0106" H 4550 4150 50  0001 C CNN
F 1 "+12V" H 4565 4473 50  0000 C CNN
F 2 "" H 4550 4300 50  0001 C CNN
F 3 "" H 4550 4300 50  0001 C CNN
	1    4550 4300
	1    0    0    -1  
$EndComp
$Comp
L power:+12V #PWR0107
U 1 1 5CA1FAFE
P 750 4300
F 0 "#PWR0107" H 750 4150 50  0001 C CNN
F 1 "+12V" H 765 4473 50  0000 C CNN
F 2 "" H 750 4300 50  0001 C CNN
F 3 "" H 750 4300 50  0001 C CNN
	1    750  4300
	1    0    0    -1  
$EndComp
Text Label 750  4700 0    50   ~ 0
LX-RX
Wire Wire Line
	750  4700 1700 4700
$Comp
L Connector:DB9_Female J1
U 1 1 5C9518E6
P 2000 4900
F 0 "J1" H 2179 4946 50  0000 L CNN
F 1 "DB9_Female" H 2179 4855 50  0000 L CNN
F 2 "JS_Local:DSUB-9_Female_Horizontal_Pitch2.77x2.54mm_EdgePinOffset9.40mm_CLICK" H 2000 4900 50  0001 C CNN
F 3 " ~" H 2000 4900 50  0001 C CNN
	1    2000 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	750  5500 1400 5500
Wire Wire Line
	1400 5300 1400 5500
Connection ~ 1400 5500
Wire Wire Line
	1400 5500 3400 5500
Text Label 10350 5000 0    50   ~ 0
COMMON-RX
Text Label 10350 5400 0    50   ~ 0
COMMON-TX
Wire Wire Line
	8750 5000 8350 5000
Wire Wire Line
	8750 5400 8350 5400
Text Label 8350 5400 0    50   ~ 0
BT-RX
Text Label 8350 5000 0    50   ~ 0
BT-TX
Wire Wire Line
	7650 5050 7900 5050
Wire Wire Line
	7650 4950 7900 4950
Text Label 7700 4950 0    50   ~ 0
BT-TX
Text Label 7700 5050 0    50   ~ 0
BT-RX
Connection ~ 9550 6200
$Comp
L power:GND #PWR0108
U 1 1 5CA4067D
P 10950 6200
F 0 "#PWR0108" H 10950 5950 50  0001 C CNN
F 1 "GND" H 10955 6027 50  0000 C CNN
F 2 "" H 10950 6200 50  0001 C CNN
F 3 "" H 10950 6200 50  0001 C CNN
	1    10950 6200
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0109
U 1 1 5CA40A18
P 10950 3600
F 0 "#PWR0109" H 10950 3450 50  0001 C CNN
F 1 "+5V" H 10965 3773 50  0000 C CNN
F 2 "" H 10950 3600 50  0001 C CNN
F 3 "" H 10950 3600 50  0001 C CNN
	1    10950 3600
	1    0    0    -1  
$EndComp
$Comp
L Device:C C3
U 1 1 5CA41016
P 8750 4150
F 0 "C3" H 8500 4200 50  0000 L CNN
F 1 "1u" H 8500 4100 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 8788 4000 50  0001 C CNN
F 3 "~" H 8750 4150 50  0001 C CNN
	1    8750 4150
	1    0    0    -1  
$EndComp
$Comp
L Device:C C4
U 1 1 5CA42392
P 10350 4150
F 0 "C4" H 10465 4196 50  0000 L CNN
F 1 "1u" H 10465 4105 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 10388 4000 50  0001 C CNN
F 3 "~" H 10350 4150 50  0001 C CNN
	1    10350 4150
	1    0    0    -1  
$EndComp
$Comp
L Device:C C5
U 1 1 5CA464BC
P 10500 4500
F 0 "C5" V 10350 4650 50  0000 C CNN
F 1 "1u" V 10450 4650 50  0000 C CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 10538 4350 50  0001 C CNN
F 3 "~" H 10500 4500 50  0001 C CNN
	1    10500 4500
	0    1    1    0   
$EndComp
$Comp
L Device:C C6
U 1 1 5CA46F54
P 10500 4800
F 0 "C6" V 10350 4950 50  0000 C CNN
F 1 "1u" V 10450 4950 50  0000 C CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 10538 4650 50  0001 C CNN
F 3 "~" H 10500 4800 50  0001 C CNN
	1    10500 4800
	0    1    1    0   
$EndComp
Wire Wire Line
	10950 4500 10950 4800
Connection ~ 10950 4800
Wire Wire Line
	10950 4800 10950 6200
NoConn ~ 10350 5200
NoConn ~ 10350 5600
NoConn ~ 8750 5600
NoConn ~ 8750 5200
$Comp
L Device:C C7
U 1 1 5CA51FC1
P 10950 3750
F 0 "C7" H 10700 3800 50  0000 L CNN
F 1 "1u" H 10700 3700 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric_Pad1.42x1.75mm_HandSolder" H 10988 3600 50  0001 C CNN
F 3 "~" H 10950 3750 50  0001 C CNN
	1    10950 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	10950 3900 10950 4500
Connection ~ 10950 4500
Wire Wire Line
	8100 3600 9550 3600
Wire Wire Line
	9550 3700 9550 3600
$Comp
L Connector:USB_A J5
U 1 1 5CA5D0AD
P 6350 4900
F 0 "J5" H 6407 5367 50  0000 C CNN
F 1 "USB_A" H 6407 5276 50  0000 C CNN
F 2 "JS_Local:USB_A_UPRIGHT_SEGOR" H 6500 4850 50  0001 C CNN
F 3 " ~" H 6500 4850 50  0001 C CNN
	1    6350 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	6250 5300 6250 5400
Wire Wire Line
	6250 5400 6350 5400
Wire Wire Line
	6650 4700 6750 4700
Wire Wire Line
	6750 4700 6750 4300
Wire Wire Line
	6350 5300 6350 5400
Connection ~ 6350 5400
Wire Wire Line
	6350 5400 6350 5500
Wire Wire Line
	10650 4500 10950 4500
Wire Wire Line
	10650 4800 10950 4800
Connection ~ 10950 6200
Wire Wire Line
	9550 6200 10950 6200
Connection ~ 10950 3600
Connection ~ 9550 3600
Wire Wire Line
	9550 3600 10950 3600
Wire Wire Line
	7250 5800 7250 6450
Wire Wire Line
	5400 5500 5800 5500
Wire Wire Line
	5800 4400 5800 4300
Wire Wire Line
	5800 4700 5800 5500
Text Notes 5300 4000 0    50   ~ 0
R-785.0-1.0 Schaltregler\n5V 1A
Text Label 6650 5000 0    50   ~ 0
COMMON-TX
Text Label 6650 4900 0    50   ~ 0
COMMON-RX
Connection ~ 6350 5500
Connection ~ 6750 4300
Connection ~ 5800 4300
Connection ~ 5800 5500
Wire Wire Line
	5800 5500 6350 5500
Wire Wire Line
	5800 4300 6750 4300
$Comp
L power:+12P #PWR0110
U 1 1 5C941CC9
P 4850 3850
F 0 "#PWR0110" H 4850 3700 50  0001 C CNN
F 1 "+12P" H 4865 4023 50  0000 C CNN
F 2 "" H 4850 3850 50  0001 C CNN
F 3 "" H 4850 3850 50  0001 C CNN
	1    4850 3850
	1    0    0    -1  
$EndComp
$Comp
L Device:Fuse F1
U 1 1 5C9D0C23
P 4850 4050
F 0 "F1" H 4910 4096 50  0000 L CNN
F 1 "Fuse 2A" H 4910 4005 50  0000 L CNN
F 2 "Fuse:Fuseholder_TR5_Littelfuse_No560_No460" V 4780 4050 50  0001 C CNN
F 3 "~" H 4850 4050 50  0001 C CNN
	1    4850 4050
	1    0    0    -1  
$EndComp
Wire Wire Line
	4850 4200 4850 4300
Wire Wire Line
	4850 3900 4850 3850
Wire Wire Line
	6350 5500 8100 5500
Wire Wire Line
	6750 4300 8100 4300
Wire Wire Line
	7650 4750 8100 4750
Wire Wire Line
	8100 4750 8100 4300
Connection ~ 8100 4300
Wire Wire Line
	7650 4850 8100 4850
Wire Wire Line
	8100 4850 8100 5500
Connection ~ 8100 5500
$Comp
L Connector:Screw_Terminal_01x03 J3
U 1 1 5CF44711
P 3750 6650
F 0 "J3" H 3850 6700 50  0000 L CNN
F 1 "Screw_Terminal_01x03" H 3850 6600 50  0000 L CNN
F 2 "TerminalBlock_MetzConnect:TerminalBlock_MetzConnect_Type059_RT06303HBWC_1x03_P3.50mm_Horizontal" H 3750 6650 50  0001 C CNN
F 3 "~" H 3750 6650 50  0001 C CNN
	1    3750 6650
	1    0    0    -1  
$EndComp
$Comp
L Connector:Screw_Terminal_01x03 J7
U 1 1 5CF44F19
P 3750 6950
F 0 "J7" H 3850 7000 50  0000 L CNN
F 1 "Screw_Terminal_01x03" H 3850 6900 50  0000 L CNN
F 2 "TerminalBlock_MetzConnect:TerminalBlock_MetzConnect_Type059_RT06303HBWC_1x03_P3.50mm_Horizontal" H 3750 6950 50  0001 C CNN
F 3 "~" H 3750 6950 50  0001 C CNN
	1    3750 6950
	1    0    0    -1  
$EndComp
$EndSCHEMATC
